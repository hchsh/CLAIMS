...
void ClientListener::configure() {
	...
	if (error2 != 0) {
		std::cout << "cannot create send thread!" << strerror(errno)
				<< std::endl;
	}
		...
		{  // OutputFdsInfo
			...
			for (int i = 0; i < MAXCONN; ++i) {
				...
				!FD_ISSET(server->m_clientFds[i], &readFds)) {
					ClientLogging::log("---%s is not in readFds",
		...
		default:
			...
			for (int i = 0; i < MAXCONN; ++i) {
				...
				if (FD_ISSET(server->m_clientFds[i], &readFds)) {
					...
					int retCode = server->receiveRequest(server->m_clientFds[i],
							buf);
					if (0 == retCode) {
						printf(
								"Successfully receive query\n %s \nfrom client %d.\n",
								buf, server->m_clientFds[i]);
					}
					//					else if (-1 == retCode) {
					//						server->shutdown();
					//						stop = true;
...
void ClientListener::generateSqlStmt(int type, char *&buf) {
	assert(buf != NULL);
	ClientLogging::log(
			"in generateSqlStmt():type argument is %d, buf argument is %s",
			type, buf);
	...
	case 1: {
		...
		string sql =
				"select max(trade_price), min(trade_price) from trade where sec_code "
						"= \'" + arg1 + "\'  and trade_date  between \' " + arg2
						+ "\' and \'" + arg3 + " \' ;";
		//		ClientLogging::log("the whole sql string is:
		//%s",sql.c_str());
		memset(buf, 0, sizeof(buf));
	...
	case 2: {
		...
		string sql =
				"select count(*) as frequency, trade_vol/100 from trade where "
						"sec_code = \'" + arg1
						+ "\' group by trade_vol/100 order by frequency desc;";
		//		ClientLogging::log("the whole sql string is:
		//%s",sql.c_str());
		memset(buf, 0, sizeof(buf));
		strcpy(buf, sql.c_str());
	...
	case 4: {
		...
		string sql =
				"select code_amt/total as proportion, sec_code "
						...
						"             and trade.trade_date between '" + arg2
						+ "' and '" + arg3
						+ "'      "
								"     ) as field_total, "
								"     ( "
		...
		string sql =
				"select sum(flow) as flow_sum, sec_field "
						...
						+ "'     "
								...
								"trade.trade_dir = 'S' "
								"						and "
								"trade.trade_date between '" + arg2 + "' and '"
						+ arg3
						+ "'         "
								"						group by "
								...
								"where trade.sec_code = field.sec_code "
								"								"
								"  and trade_date between '" + arg2 + "' and '"
						+ arg3
						+ "'     "
								"								"
								"group by trade.trade_no, trade.sec_code, field.sec_field "
	...
	case 7: {
		...
		string sql =
				"select (avg(t2.trade_price) - "
						...
						"and t1.trade_date between \'" + arg2 + "\' and \'"
						+ arg3 + "\' "
								"and t2.trade_date between \'" + arg2
						+ "\' and \'" + arg3 + "\' "
		...
		if (result.status_ == true) {
			...
			if (result.result_ == NULL) {
				...
				if (client_type::java == server->client_type_) {
					...
					ClientListenerLogging::log(
							...
							cliRes.content.c_str(), result.fd_);
					server->write(result.fd_, cliRes);
					...
					std::vector<std::string> list =
							result.result_->column_header_list_;
					ColumnHeader header;
					...
					while (block = (BlockStreamBase *) it.atomicNextBlock()) {
						...
						ClientListenerLogging::log(
								"to send data response-- status:%d  length:%d  content:%s  "
										"fd:%d", cliRes.status, cliRes.length,
								cliRes.content.c_str(), result.fd_);
						server->write(result.fd_, cliRes);
					}
			...
			ClientListenerLogging::log(
					"to send err response-- status:%d  length:%d  content:%s"
							"warnings: %s", cliRes.status, cliRes.length,
					cliRes.content.c_str(), result.warning_.c_str());
			server->write(result.fd_, cliRes);
			...
			while (block = it.nextBlock()) {
				...
				while (tuple = block_it->nextTuple()) {
					...
					for (int i = 0; i < rs->schema_->getncolumns(); ++i) {
						jv_temp[col_name_list[i]] = Json::Value(
								rs->schema_->getColumnValue(tuple, i));
					}
					root.append(jv_temp);
				}
			}
